...
pub fn list_repository_history(workspace_path: &Path, limit: usize) -> Result<Vec<GitRepositoryCommit>, GitHistoryError> {
    list_repository_history_page(workspace_path, limit, 0)
}

pub fn list_repository_history_page(workspace_path: &Path, limit: usize, offset: usize) -> Result<Vec<GitRepositoryCommit>, GitHistoryError> {
    ...
    let output = run_git(&repository_root, &[
        ...
        format!("--skip={offset}"), "-n".into(), limit.to_string(),
    ])?;
