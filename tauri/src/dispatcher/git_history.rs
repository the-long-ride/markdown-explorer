...
pub use git_repository::{
    GitRepositoryCommit, GitRevisionFile, GitRevisionFileSnapshot,
    list_repository_history, list_repository_history_page, list_revision_files, read_revision_file,
};
...
pub(super) fn handle_command(app: &tauri::AppHandle, state: &crate::app_state::AppState, cmd: &str, msg: &serde_json::Value) -> Result<bool, String> {
    ...
    match cmd {
        ...
        "listRepositoryHistory" => {
            let result = workspace_path.as_deref().ok_or_else(|| error("not-repository", "No workspace"))
                .and_then(|root| list_repository_history_page(
                    root,
                    msg.get("limit").and_then(serde_json::Value::as_u64).unwrap_or(DEFAULT_HISTORY_LIMIT as u64) as usize,
                    msg.get("offset").and_then(serde_json::Value::as_u64).unwrap_or(0) as usize,
                ));
            extra.insert("ok".into(), result.is_ok().into());
