...
The capability-aware **History** sidebar is enabled by default but appears only when local Git is supported for the active workspace. **More Actions → History** expands the sidebar, selects History, and lets the visible panel start lazy loading.

Repository History is paginated instead of loading one fixed prefix forever:

- the initial page requests **50 commits**;
- later pages use `listRepositoryHistory(limit, offset)` with `offset` equal to the number already loaded;
- Electron, VS Code, and Tauri translate the offset to Git `--skip=<offset>` and the limit to `-n <limit>`;
- returned pages are appended in order and de-duplicated by full object ID;
- a short page marks history exhausted;
- an intersection sentinel loads older commits near the bottom, with a scroll fallback for environments without `IntersectionObserver`.

The History search box filters loaded commits case-insensitively by full SHA, short SHA, author, or subject/message. If a query has no match in the loaded page and older history remains, Markdown Explorer loads additional pages until it finds a match or exhausts history.

The commit graph uses one continuous graph layer for the visible list. Vertical lanes and merge curves remain connected between rows instead of restarting inside each commit row. Commit rows show subject, short SHA, author, refs, merge-parent relationships, and the exact `HEAD` marker.
...
- revision Files uses normal document eligibility: Markdown/MDX and TXT are included, convertible document types are included only when document conversion is enabled, and arbitrary source files such as `.ts` are excluded;
...
- historical content is rendered read-only while live tabs, split panes, dirty document sessions, and scroll state stay mounted underneath;
...
The selected historical revision is persisted **per workspace**, preferring the workspace path as its key. Reopening that workspace restores the same revision when the object still exists. A persisted revision is not discarded merely because it is absent from the first 50-commit page; validation can page older history. If the object is genuinely unavailable after validation, for example after rebase or prune, Markdown Explorer clears the stale selection and falls back to live HEAD.
...
Document-scoped history remains available for revision/diff actions. History is loaded lazily so normal document navigation does not start Git or scan commit history.

Each revision shows the commit subject, author, time, short object ID, and the path used by that revision. Rename history is followed backwards so snapshots continue to resolve before a file rename.
...
A split pane can independently display **Rendered**, **Inline Edit**, **Plain**, **Revision**, or **Diff** modes. Revision and Diff modes are read-only.

Each pane owns its own tab collection and active tab. Normal navigation targets the focused pane; closing split view preserves the focused pane and its tab strip. Ordinary pane scrolling remains local during the scroll interaction and is persisted at document/pane lifecycle boundaries so the opposite pane is not invalidated on every wheel/trackpad event.
...
Request IDs are correlated in the shared History client so concurrent responses cannot resolve the wrong request. Pagination uses the same `repositoryHistoryResult` response type for each page; the renderer owns append/de-duplication state. UI request generations and revision-search abort signals also prevent stale historical reads from replacing a newer workspace/revision view.
