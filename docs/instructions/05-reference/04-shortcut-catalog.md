...
| `toggleWorkspaceInsights` | — (Chromium/Web) | — | Ctrl+Alt+I |
...
- `toggleWorkspaceInsights` toggles the Workspace Insights panel from the desktop More actions surface (default `Ctrl+Alt+I`). Chromium can expose the panel from More actions but has no default keyboard binding; the website demo exposes neither action nor shortcut, even when an imported setting enables Insights.
