...
- `repositoryHistoryResult` is also the page response for `listRepositoryHistory(limit, offset)`. Pagination does not introduce a second response type.
- A successful repository page returns commit metadata including parent OIDs, refs/decorations, and `isHead`; it never means the host checked out that commit.
- The renderer appends pages in request order, de-duplicates commits by full OID, and treats a page shorter than the requested limit as history exhaustion.
- Persisted-revision restore may request additional repository pages; a saved OID is not invalidated simply because it is absent from the initial 50-commit page.
...
```typescript
const message = {
  command: 'repositoryHistoryResult',
  requestId: 'history-page-3',
  ok: true,
  commits: [],
};
```
...
| Implementation | `ui/src/contexts/RepositorySnapshotContext.tsx` | Page append/de-duplication, exhaustion, and persisted-revision restore |
...
| Verification | `tests/node/repository-history-pagination-contract.test.mjs` | Cross-runtime pagination behavior |
