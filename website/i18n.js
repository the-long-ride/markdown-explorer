window.LANGS = {
  ...
  fr: {
    ...
    downloadSubtitle:
      "L'application de bureau va plus loin que la version extension VS Code avec l'affichage par onglets, l'ouverture de plusieurs espaces de travail en même temps, ainsi que davantage de raccourcis clavier et de fonctionnalités. Les versions légères utilisent Tauri pour un bundle plus petit avec toutes les fonctionnalités.",
    dlVscodeTitle: "VS Code Marketplace",
    dlVscodeBody:
    ...
    dlWinBody:
      "Utilisez l’installateur NSIS pour les raccourcis du menu Démarrer et du bureau, ou le `.exe` portable pour l’exécuter directement sans installation.",
    ...
    dlLinuxBody:
      "Utilisez la version Electron `.AppImage` pour un lancement portable, ou `.deb` sur les systèmes Debian et Ubuntu. Les versions légères sont disponibles en AppImage ou en `.deb`.",
    dlLinuxBtn: "Télécharger Linux",
    dlLinuxAppImageBtn: "AppImage",
    dlLinuxDebBtn: ".deb",
    dlWinLightweightBtn: "Version légère",
    dlLinuxLightweightAppImageBtn: "Version légère AppImage",
    dlLinuxLightweightDebBtn: "Version légère .deb",
  ...
  es: {
    ...
    downloadSubtitle:
      "La app de escritorio ofrece más que la versión de extensión para VS Code: vista por pestañas, varios espacios de trabajo abiertos al mismo tiempo, y más atajos de teclado y funciones. Las versiones ligeras usan Tauri para un paquete más pequeño con todas las funciones.",
    dlVscodeTitle: "VS Code Marketplace",
    dlVscodeBody:
    ...
    dlWinBody:
      "Usa el instalador NSIS para accesos directos en el menú Inicio y el escritorio, o el `.exe` portable para ejecutarlo directamente sin instalación.",
    ...
    dlLinuxBody:
      "Usa la versión Electron `.AppImage` para lanzamiento portable, o `.deb` en sistemas Debian y Ubuntu. Las versiones ligeras están disponibles en AppImage o `.deb`.",
    dlLinuxBtn: "Descargar Linux",
    dlLinuxAppImageBtn: "AppImage",
    dlLinuxDebBtn: ".deb",
    dlWinLightweightBtn: "Versión ligera",
    dlLinuxLightweightAppImageBtn: "Versión ligera AppImage",
    dlLinuxLightweightDebBtn: "Versión ligera .deb",
  ...
  zh: {
    ...
    downloadSubtitle:
      "桌面应用比 VS Code 扩展版本支持更多功能：标签页视图可同时打开多个工作区，并提供更多键盘快捷键和功能。轻量版使用 Tauri，包体更小但功能完整。",
    ...
    dlWinBody:
      "使用 NSIS 安装版可创建开始菜单和桌面快捷方式，或下载便携式 `.exe` 直接运行而无需安装。",
    ...
    dlLinuxBody:
      "可使用 Electron `.AppImage` 便携运行，或在 Debian 和 Ubuntu 系统上使用 `.deb`。轻量版提供 AppImage 和 `.deb` 两种格式。",
  ...
  no: {
    ...
    downloadSubtitle:
      "Skrivebordsappen støtter mer enn VS Code-utvidelsen med fanevisning, flere arbeidsområder åpne samtidig, samt flere tastatursnarveier og funksjoner. Lettvektsversjoner bruker Tauri for mindre pakkestørrelse med full funksjonalitet.",
    dlVscodeTitle: "VS Code Marketplace",
    dlVscodeBody:
    ...
    dlWinBody:
      "Bruk NSIS-installatøren for Start-meny- og skrivebordssnarveier, eller den bærbare `.exe`-filen for å kjøre direkte uten installasjon.",
    dlWinBtn: "Last ned Windows",
    dlWinNsisBtn: "Installasjonsprogram",
    dlWinPortableBtn: "Portabel",
    ...
    dlLinuxBody:
      "Bruk Electron `.AppImage` for bærbar oppstart, eller `.deb` på Debian- og Ubuntu-baserte systemer. Lettvektsversjoner er tilgjengelige som AppImage eller `.deb`.",
    dlLinuxBtn: "Last ned Linux",
    dlLinuxAppImageBtn: "AppImage",
    dlLinuxDebBtn: ".deb",
    dlWinLightweightBtn: "Lettvektsversjon",
    dlLinuxLightweightAppImageBtn: "Lettvekts AppImage",
    dlLinuxLightweightDebBtn: "Lettvekts .deb",
    dlMacLightweightBtn: "Lettvektsversjon",
  ...
  ja: {
    ...
    downloadSubtitle:
      "デスクトップアプリは VS Code 拡張機能版より多くをサポートし、タブ表示で複数のワークスペースを同時に開けるほか、より多くのキーボードショートカットと機能を利用できます。軽量版は Tauri を使い、バンドルサイズが小さくても機能はフルです。",
    ...
    dlWinBody:
      "スタートメニューやデスクトップのショートカットには NSIS インストーラーを使い、インストール不要で直接実行したい場合はポータブル `.exe` を使います。",
    ...
    dlLinuxBody:
      "Electron の `.AppImage` はポータブル起動向け、`.deb` は Debian / Ubuntu 系向けです。軽量版は AppImage と `.deb` を用意しています。",
    dlLinuxBtn: "Linux をダウンロード",
    dlLinuxAppImageBtn: "AppImage",
    dlLinuxDebBtn: ".deb",
    dlWinLightweightBtn: "軽量版",
    dlLinuxLightweightAppImageBtn: "軽量版 AppImage",
    dlLinuxLightweightDebBtn: "軽量版 .deb",
    dlMacLightweightBtn: "軽量版",
    dlChromiumTitle: "Chromium 拡張機能",
    dlChromiumBody:
      "ブラウザで直接 Markdown Explorer を実行します。最新リリースから展開済みの拡張機能をインストールします。",
    dlChromiumBtn: "拡張機能 zip",
    dlGhTitle: "最新リリースページ",
    dlGhBody:
  ...
  ko: {
    ...
    downloadSubtitle:
      "데스크톱 앱은 VS Code 확장 버전보다 더 많은 기능을 지원합니다. 탭 보기로 여러 워크스페이스를 동시에 열 수 있고, 더 많은 키보드 단축키와 기능을 제공합니다. 경량 빌드는 Tauri를 사용해 번들 크기가 더 작지만 기능은 그대로입니다.",
  ...
  ru: {
    ...
    downloadSubtitle:
      "Настольное приложение поддерживает больше возможностей, чем версия расширения для VS Code: вкладки, одновременное открытие нескольких рабочих областей, а также больше сочетаний клавиш и функций. Лёгкие версии используют Tauri для меньшего размера пакета при полном наборе функций.",
    dlVscodeTitle: "VS Code Marketplace",
    dlVscodeBody:
