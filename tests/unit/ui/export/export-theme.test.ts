...
afterEach(() => {
  ...
  document.head.querySelectorAll('style[data-export-theme-test]').forEach((style) => style.remove());
});
...
describe('captureExportThemeSnapshot', () => {
  ...
  it('keeps content/runtime selectors and font faces while dropping app-shell and settings rules', () => {
    ...
    style.dataset.exportThemeTest = 'true';
    style.textContent = `
  ...
  it('keeps shared app switch styles required by exported table column controls', () => {
    const style = document.createElement('style');
    style.dataset.exportThemeTest = 'true';
    style.textContent = `
      .app-switch { width: 32px; height: 18px; }
      .app-switch__thumb { width: 12px; height: 12px; }
      .app-switch.is-on .app-switch__thumb { transform: translateX(14px); }
    `;
    document.head.appendChild(style);

    const snapshot = captureExportThemeSnapshot();

    expect(snapshot.cssText).toContain('.app-switch');
    expect(snapshot.cssText).toContain('.app-switch__thumb');
    expect(snapshot.cssText).toContain('translateX(14px)');
  });
});
