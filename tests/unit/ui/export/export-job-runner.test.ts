...
function job(format: ExportJob['format'], files: MdFile[], batchMode: ExportJob['batchMode'] = 'separate', extras: string[] = []): ExportJob {
  return { format, layout: 'document', batchMode, files, extraResourcePaths: extras };
}
...
describe('runExportJob', () => {
  ...
  it('fails before save when an explicitly selected extra cannot be read', async () => {
    const target = file('Guide');
    const save = vi.fn();
    const result = await runExportJob({ ...baseArgs, runtime: 'chrome', job: job('html', [target], 'separate', ['data/missing.json']) }, dependencies({
      loadSnapshot: vi.fn(async () => snapshot(target)), saveArtifact: save,
      readResource: vi.fn(async () => ({ ok: false as const, reason: 'unreadable' as const })),
    }));
    expect(result.failureCount).toBe(1);
    expect(save).not.toHaveBeenCalled();
  });
...
describe('exportArtifactLabel', () => {
  it('predicts package/file shape before export', () => {
    ...
    expect(exportArtifactLabel({ format: 'html', batchMode: 'merged', documentCount: 1, extraCount: 1 })).toBe('HTML package (.zip)');
