...
function file(): MdFile {
  return {
    fsPath: '/docs/guide.md', relativePath: 'guide.md', parts: ['guide.md'], fileName: 'guide.md',
    title: 'Guide', extension: '.md', documentKind: 'markdown',
  };
}
...
describe('export feature detection', () => {
  ...
  it('detects portable media and HTML preview features without requiring Mermaid runtime JS', () => {
    const root = createRoot('<img src="image.png"><div class="mdn-mermaid-wrap"><div class="mermaid" data-mdn-rendered="true"><svg></svg></div></div><div class="mdn-html-preview-wrap"><iframe class="mdn-html-preview-iframe"></iframe></div>');
  ...
  it('detects enhanced data tables and adds charts only when the chart view switcher exists', () => {
    ...
    const chartable = createRoot('<div class="mdn-table-wrap"><div class="mdn-table-view-dropdown"></div><table class="mdn-table" data-mdn-enhanced="true"></table><div class="mdn-table-chart-container"><canvas></canvas></div></div>');
...
describe('enhanced export snapshots', () => {
  it('enhances in a connected offscreen host, captures PDF visuals before cleanup, and serializes stable visual ids', async () => {
    const snapshot: DocumentSnapshot = {
      file: file(), markdownSource: '# Guide',
      html: '<p id="target">Guide</p><img id="hero" alt="Hero" src="data:image/png;base64,AA==">',
    };
    const enhanced = await enhanceExportSnapshot(snapshot, {
      ...
      enhance: async ({ body }) => {
        ...
        body.insertAdjacentHTML('beforeend', '<div class="mdn-html-preview-wrap"><iframe class="mdn-html-preview-iframe" srcdoc="<p>Preview</p>"></iframe></div><div class="mdn-mermaid-wrap"><div class="mermaid" data-mdn-rendered="true"><svg width="100" height="40"></svg></div></div>');
      },
    });
    expect(enhanced.html).toContain('data-enhanced="true"');
    expect(enhanced.html).toContain('data-mdn-pdf-visual-id="pdfv-1"');
...
describe('mapWithConcurrency', () => {
  it('preserves input ordering while never exceeding the requested concurrency', async () => {
    ...
    const settled = await mapWithConcurrency([20, 5, 10, 1], 2, async (delay) => {
      ...
      active -= 1; return `done-${delay}`;
    });
    expect(maximum).toBe(2);
    expect(settled).toEqual([
      { status: 'fulfilled', value: 'done-20' }, { status: 'fulfilled', value: 'done-5' },
      { status: 'fulfilled', value: 'done-10' }, { status: 'fulfilled', value: 'done-1' },
    ]);
  });
