...
describe('buildStandaloneExportHtml', () => {
  ...
  it('builds Explorer layout with topbar, collapsible file tree, and right table of contents', () => {
    const navigationFiles = [
      file('guide/z.md'),
      file('guide/deep/a.md'),
      file('guide/b.md'),
      file('root.md'),
    ];
    const html = buildStandaloneExportHtml({
      pages: [{
        file: navigationFiles[0],
        html: '<section class="mdn-section mdn-section--h1" id="overview"><div class="mdn-section-header"><h1 class="mdn-section-title">Overview</h1></div></section><section class="mdn-section mdn-section--h2" id="setup"><div class="mdn-section-header"><h2 class="mdn-section-title">Setup</h2></div></section>',
      }],
      navigationFiles,
      layout: 'explorer',
      title: 'Docs',
    ...
    expect(html).toContain('class="mdn-export-tree"');
    expect(html).toContain('<details class="mdn-export-tree-folder" open>');
    expect(html).toContain('aria-label="On this page"');
    expect(html).not.toContain('aria-label="Files"');
    expect(html).toContain('<div class="mdn-export-pane-title">On this page</div>');
    expect(html).toContain('class="mdn-export-toc__link is-level-1" href="#overview">Overview</a>');
    expect(html).toContain('class="mdn-export-toc__link is-level-2" href="#setup">Setup</a>');
    expect(html).toContain('class="mdn-export-tree-file is-active" aria-current="page" href="#doc-guide-z-md"');
    expect(html).toContain('href="deep/a.md.html"');
    expect(html).toContain('href="b.md.html"');
    expect(html).toContain('href="../root.md.html"');

    const zIndex = html.indexOf('>z.md</a>');
    const deepIndex = html.indexOf('<summary>deep</summary>');
    const bIndex = html.indexOf('>b.md</a>');
    expect(zIndex).toBeGreaterThan(-1);
    expect(deepIndex).toBeGreaterThan(zIndex);
    expect(bIndex).toBeGreaterThan(deepIndex);

    const asideIndex = html.indexOf('aria-label="On this page"');
    const mainIndex = html.indexOf('<main class="mdn-export-main">');
    expect(asideIndex).toBeGreaterThan(mainIndex);
  });

  it('uses merged document anchors and groups the table of contents per document', () => {
    const richPages = [
      { file: source, html: '<section class="mdn-section mdn-section--h1" id="intro-heading"><div class="mdn-section-header"><h1 class="mdn-section-title">Intro</h1></div></section>' },
      { file: target, html: '<section class="mdn-section mdn-section--h2" id="api-heading"><div class="mdn-section-header"><h2 class="mdn-section-title">API</h2></div></section>' },
    ];
    const html = buildStandaloneExportHtml({ pages: richPages, layout: 'explorer', title: 'Docs', themeCss: '' });
    expect(html).toContain('href="#doc-guide-intro-md"');
    expect(html).toContain('href="#doc-reference-api-md"');
    expect(html.match(/aria-label="On this page"/g)).toHaveLength(1);
    expect(html).toContain('class="mdn-export-toc__doc" href="#doc-guide-intro-md"');
    expect(html).toContain('class="mdn-export-toc__link is-level-2" href="#api-heading">API</a>');
  });

  it('omits the table of contents pane when exported documents expose no headings', () => {
    const html = buildStandaloneExportHtml({
      pages: [{ file: source, html: '<p>No headings here</p>' }, { file: target, html: '<p>None either</p>' }],
      layout: 'explorer',
      title: 'Docs',
      themeCss: '',
    });
