...
describe('LinkContextMenu scope action', () => {
  ...
  it('uses the supplied scope target icon for Open as scope', () => {
    render(
      <LinkContextMenu
        state={{ x: 20, y: 20, link }}
        menuLabel="Link menu"
    ...
    const button = screen.getByText('Open as scope').closest('button');
    const svg = button?.querySelector('svg');
    expect(svg?.getAttribute('viewBox')).toBe('0 0 122.881 122.88');
    expect(svg?.querySelector('path')?.getAttribute('d')).toContain('M61.44,35.174');
  });
