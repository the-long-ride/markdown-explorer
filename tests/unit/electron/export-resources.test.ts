...
// skip symlink-specific coverage instead of failing on environment limits.
const canSymlink = (() => {
  try {
    const probe = join(mkdtempSync(join(tmpdir(), 'mdn-symlink-probe-')), 'probe');
...
describe('Electron export resource handlers', () => {
  ...
  it.skipIf(!canSymlink)('rejects symlink escapes after canonical resolution', () => {
    const ws = workspace();
