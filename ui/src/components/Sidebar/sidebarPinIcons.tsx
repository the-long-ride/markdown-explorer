...
// "Clear all pins" toolbar action. Reuses the Lucide thumbtack + diagonal
// slash overlay from `UnpinIcon` so both "unpin" affordances (per-item menu
// and clear-all toolbar) stay visually identical — the user sees one
// consistent "remove pin" affordance regardless of whether they are removing
// a single pinned item or every pinned item at once. Kept as a distinct
// named export so the call-site in `SidebarFilesActions.tsx` reads as
// "clear all pins" rather than "unpin this item"; the rendering is shared.
