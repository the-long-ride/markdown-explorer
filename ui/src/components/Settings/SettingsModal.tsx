...
import { useAppState, DEFAULT_KEYBINDINGS } from '../../contexts/AppStateContext';
...
const ACTIONS_LIST = [
  ...
  { id: 'workspaceSelection', label: 'Go to workspace selection', scope: 'desktop' },
...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const visibleActions = ACTIONS_LIST.filter(act => act.scope === 'both' || isElectron);

  const handleKeyDown = (actionId: string, e: React.KeyboardEvent<HTMLInputElement>) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const nextBindings = { ...state.settings.keybindings, [actionId]: shortcutStr };
  ...
  return (
    ...
    >
      <div className="settings-card" style={{ width: '450px', maxWidth: '90%' }}>
        <button
          className="settings-card__close tooltip-container"
          onClick={onClose}
          aria-label="Close"
          data-tooltip-pos="below"
        >
          &times;
          <span className="tooltip-text">Close Settings [Esc]</span>
        </button>
        ...
        <div className="settings-card__body">
          ...
          <div className="settings-item">
            <div className="settings-item__info">
              ...
              <div className="settings-item__desc">
                Show document titles/H1 headers instead of raw filenames in the sidebar tree.
              </div>
            </div>
            <label className="switch-toggle" aria-label="Toggle sidebar file labels">
              <input
                type="checkbox"
          ...
          <div className="settings-item" style={{ marginTop: 12, borderTop: '1px solid var(--bd)', paddingTop: 12 }}>
            <div className="settings-item__info">
              <div className="settings-item__title">Default HTML Code Block View</div>
              <div className="settings-item__desc">
                Show HTML code blocks as interactive previews by default. Otherwise, shows the raw HTML code.
              </div>
            </div>
            <label className="switch-toggle" aria-label="Toggle default HTML view">
              <input
                type="checkbox"
                checked={state.settings.defaultHtmlPreview}
                onChange={(e) => updateSettings({ defaultHtmlPreview: e.target.checked })}
              />
              <span className="switch-slider" />
            </label>
          </div>
          ...
          <div style={{ marginTop: 16, borderTop: '1px solid var(--bd)', paddingTop: 16 }}>
            <div style={{ fontWeight: 600, fontSize: '13.5px', marginBottom: 12, color: 'var(--tx)' }}>
              Keyboard Shortcuts
            </div>
            <div
              style={{
            ...
            >
              {visibleActions.map((act) => {
                ...
                return (
                  ...
                  >
                    <div style={{ color: 'var(--tx2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span>{act.label}</span>
                    </div>
                    <input
                      type="text"
                      ...
                      value={isRecording ? 'Press keys...' : val.split('+').join(' + ')}
                      ...
                      style={{
                        width: '130px',
                        textAlign: 'center',
                        background: isRecording ? 'rgba(109, 94, 240, 0.15)' : 'var(--bg-s)',
                        border: isRecording ? '1px solid var(--accent)' : '1px solid var(--bd-s)',
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
              <button
                onClick={() => updateSettings({ keybindings: DEFAULT_KEYBINDINGS })}
                style={{
                  background: 'none',
                  border: 'none',
                  color: '#6d5ef0',
