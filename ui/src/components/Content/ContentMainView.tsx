...
interface ContentMainViewProps {
  ...
  isDesktopTabView: boolean;
  isUnavailableWorkspaceInHistory: boolean;
  suppressWelcome: boolean;
  hasRenderableDocumentContent: boolean;
  isHtmlDocument: boolean;
  sourceDocumentText: string | null;
  htmlMarkdownRender: { html: string; error: string | null };
  htmlDocumentPreviewEnabled: boolean;
  previewTitle: string;
  previewWarning: string;
  previewMeta: string;
  frontmatterEntries: Array<[string, string]>;
  renderedContentParts: { leadingCommentsHtml: string; bodyHtml: string };
  onCancelWorkspaceScan?: () => void;
  onOpenWorkspaceAgain: () => void;
  onDeleteUnavailableWorkspace: () => void;
      ...
      <div className={`content__scroll${isFullHtmlPreview ? ' content__scroll--html-preview' : ''}`} id="contentScroll" ref={scrollRef}>
        ...
        {!state.isLoading && workspaceUnavailablePath && (
          <div className="state-screen state-screen--workspace-unavailable">
            ...
            <div className="state-screen__sub">{t.workspaceUnavailable.description}</div>
            <div className="state-screen__path">{workspaceUnavailablePath}</div>
            {isDesktopTabView && <div className="state-screen__hint">{t.workspaceUnavailable.tabHint}</div>}
            <div className="state-screen__actions">
              <button type="button" className="state-screen__button state-screen__button--primary" onClick={onOpenWorkspaceAgain}>
                <FolderIcon size={14} /><span>{t.workspaceUnavailable.openAgain}</span>
              </button>
              <button type="button" className="state-screen__button state-screen__button--danger" onClick={onDeleteUnavailableWorkspace} disabled={!isUnavailableWorkspaceInHistory}>
                <TrashIcon size={14} />
                <span>{isUnavailableWorkspaceInHistory ? t.workspaceUnavailable.deleteHistory : t.workspaceUnavailable.removedHistory}</span>
              </button>
            </div>
          </div>
        )}
        ...
        {!state.isLoading && !state.notFoundHref && !workspaceUnavailablePath && state.currentFile && hasRenderableDocumentContent && (
          <div className={`mdn-body${isFullHtmlPreview ? ' mdn-body--html-preview' : ''}`} id="mdBody" ref={bodyRef} aria-live="polite">
            {state.staleContentFilePath === state.currentFile && (
              <div className="document-preview-notice current-file-change-notice" role="status">
                ...
                <div className="document-preview-notice__body current-file-change-notice__body">
                  ...
                  <button type="button" className="btn current-file-change-notice__button" onClick={onRefresh}>{previewCopy.refreshCurrentFile}</button>
            ...
            {isHtmlDocument && sourceDocumentText !== null ? (
              ...
              <>
                {previewInfo && (
                  <div className={`document-preview-notice document-preview-notice--${previewInfo.kind}`} role="note">
                    <AlertTriangleIcon size={16} />
                    ...
                    <div className="mdn-frontmatter-grid">
                      {frontmatterEntries.map(([key, value]) => (
                        <div className="mdn-frontmatter-field" key={key}>
                          ...
                          <span className={`mdn-frontmatter-value${value ? '' : ' is-empty'}`}>{value || '\u00a0'}</span>
                        </div>
                      ))}
                    </div>
                  </details>
