...
interface LinkRow { readonly key: string; readonly sourcePath: string; readonly target: string; readonly status: InsightsPresentationStatus; readonly broken: boolean; readonly checkedAt?: string; }
...
export function LinksView({ snapshot, documents, labels: suppliedLabels, externalResults, externalCheckingEnabled = false, externalChecking = false, onCheckExternalLinks, onCancelExternalChecks }: LinksViewProps) {
  ...
  const rows = useMemo<LinkRow[]>(() => {
    const result: LinkRow[] = snapshot.brokenLinks.map(link => ({ key: `broken:${link.sourcePath}:${link.sourceStart}`, sourcePath: link.sourcePath, target: `${link.target}${link.fragment ? `#${link.fragment}` : ''}`, status: link.status as InsightsPresentationStatus, broken: true }));
    for (const document of documents) {
      for (const dynamic of document.dynamicReferences) result.push({ key: `dynamic:${document.path}:${dynamic.sourceStart}`, sourcePath: document.path, target: `{${dynamic.expression}}`, status: 'dynamic', broken: false });
      for (const reference of document.references) {
        if (!reference.remote) continue;
        const checked = externalResults.get(reference.target);
        result.push({ key: `remote:${document.path}:${reference.sourceStart}`, sourcePath: document.path, target: reference.target, status: (checked?.status ?? 'unchecked') as InsightsPresentationStatus, broken: checked?.status === 'broken' || checked?.status === 'unreachable', checkedAt: checked?.checkedAt });
      }
    }
  ...
  return (
    <div className="insights-links">
      ...
      <ul className="insights-list">{rows.map(row => <li key={row.key} className="insights-list__row"><div><strong>{row.target}</strong><span>{row.sourcePath}{row.checkedAt ? ` · ${new Date(row.checkedAt).toLocaleString()}` : ''}</span></div><span className={`insights-status insights-status--${row.status}`} data-status={row.status}>{insightsStatusLabel(labels, row.status)}</span></li>)}</ul>
    </div>
  );
}
