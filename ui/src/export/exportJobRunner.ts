...
import { listWorkspaceExportResources, readWorkspaceExportResource } from './exportResources';
...
export interface ExportJobRunnerDependencies {
  ...
  listResources: typeof listWorkspaceExportResources;
...
const DEFAULT_DEPENDENCIES: ExportJobRunnerDependencies = {
  ...
  listResources: listWorkspaceExportResources,
...
export async function runExportJob(
  ...
  if (cancelled()) { result.cancelled = true; return result; }
  try {
    let extras: readonly ExportAsset[] = [];
    if (args.job.extraResourcePaths.length > 0) {
      const resources = args.resources ?? await deps.listResources(args.bridge);
      extras = await collectExplicitExportAssets({
        selectedPaths: args.job.extraResourcePaths,
        resources,
        readResource: resourceReader,
      });
    }
    const mergedAssets = mergeExportAssets(referencedAssets, extras);
    ...
    const artifact = args.job.format === 'site'
      ...
      : deps.composeHtml({
          ...
          theme,
          runtimeAssets,
          extras: mergedAssets.filter((asset) => asset.kind === 'extra'),
        });
    if (cancelled()) { result.cancelled = true; return result; }
    const saved = await deps.saveArtifact(args.bridge, args.runtime, {
      fileName: artifact.fileName,
