...
function captureElement(element: Element, kind: PdfVisualKind, id: string): PdfVisualBlock {
  ...
  if (kind === 'mermaid') {
    ...
    return svg ? { id, kind, svg: sanitizeSvg(svg), ...size } : { id, kind, fallbackText: 'Mermaid diagram unavailable' };
  }
  if (kind === 'chart') {
    ...
    return image ? { id, kind, image, ...size } : { id, kind, fallbackText: 'Chart preview unavailable', warning: 'Chart canvas could not be captured.' };
  }
  if (kind === 'image') {
    const imageElement = element as HTMLImageElement;
    const image = imageDataUrl(imageElement);
    return image ? { id, kind, image, ...size } : { id, kind, fallbackText: imageElement.alt || 'Image unavailable', warning: 'Image could not be captured for PDF.' };
  }
