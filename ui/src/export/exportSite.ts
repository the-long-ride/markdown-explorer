...
export function composeHtmlExport(args: {
  ...
  if (merged) {
    ...
    return { kind: 'html', fileName: `${args.baseName}-merged.html`, bytes: encoder.encode(html) };
  }
  ...
  const entries = uniqueEntries([...pages]);
...
export function composeStaticSiteExport(args: {
  ...
  entries.push(...runtimeEntries, ...assetEntries(args.referencedAssets));
