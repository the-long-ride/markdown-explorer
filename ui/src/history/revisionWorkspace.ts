...
const MARKDOWN_DOCUMENT_EXTENSION = /\.(?:md|mdx)$/i;
const TEXT_DOCUMENT_EXTENSION = /\.txt$/i;
const CONVERTIBLE_DOCUMENT_EXTENSION = /\.(?:doc|docx|pdf|html?|xls|xlsx|xlm|pptx|odt|odp|ods|rtf)$/i;
const SEARCHABLE_TEXT_EXTENSION = /\.(?:md|mdx|txt|html?)$/i;
...
export function isRevisionDocumentPath(path: string, documentConversionEnabled = false): boolean {
  const normalized = normalizeRevisionPath(path);
  return MARKDOWN_DOCUMENT_EXTENSION.test(normalized)
    || TEXT_DOCUMENT_EXTENSION.test(normalized)
    || (documentConversionEnabled && CONVERTIBLE_DOCUMENT_EXTENSION.test(normalized));
}
...
export function buildRevisionWorkspaceProjection(
  ...
  for (const entry of files) {
    ...
    if (!relativePath || !isRevisionDocumentPath(relativePath, documentConversionEnabled)) continue;
    ...
    const file: MdFile = {
      ...
      extension,
      documentKind: MARKDOWN_DOCUMENT_EXTENSION.test(extension) ? 'markdown' : 'document',
      modifiedAt: 0,
    };
    fileList.push(file);
...
export async function searchRevisionWorkspace(
  ...
  const searchable = files
    .filter((file) => isRevisionDocumentPath(file.path, documentConversionEnabled) && SEARCHABLE_TEXT_EXTENSION.test(file.path))
    .slice(0, MAX_REVISION_SEARCH_FILES);
