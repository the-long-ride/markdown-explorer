...
function renderItem(container: HTMLElement, item: PortableMediaItem, scale: number, panX: number, panY: number): HTMLElement {
  ...
  const media = item.kind === 'image' ? doc.createElement('img') : doc.createElement('div');
  if (item.kind === 'image') {
...
export function openPortableMediaViewer(
  ...
  const entries = mediaElements(doc)
    .map((element) => ({ element, item: itemFromElement(element) }))
    .filter((entry): entry is { element: Element; item: PortableMediaItem } => entry.item !== null);
  const initialIndex = entries.findIndex((entry) => entry.element === source);
  if (initialIndex < 0 || entries.length === 0) return null;
