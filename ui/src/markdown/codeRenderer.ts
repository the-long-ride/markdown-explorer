...
import { getTranslations } from '../contexts/translations.ts';
import type { Translations } from '../contexts/translationTypes.ts';
...
function sourceAttributes(token: CodeBlockToken, kind: 'code' | 'mermaid'): string {
  const start = token.sourceStart ?? 0;
  const end = token.sourceEnd ?? start;
  const identity = kind === 'mermaid'
    ? ` data-mdn-mermaid-source="${encodeURIComponent(token.content.trim())}"`
    : '';
  return `data-mdn-source-start="${start}" data-mdn-source-end="${end}" data-mdn-bookmark-kind="${kind}"${identity}`;
}
...
export function renderCodeBlock(token: CodeBlockToken, themeOrOptions: string | CodeRendererOptions): string {
  ...
  if (isMermaid) {
    return `<div class="mdn-mermaid-wrap" ${sourceAttributes(token, 'mermaid')}>
