...
export interface ExportCenterFeatureTranslations {
  ...
  source: {
    ...
    documentCount: string; renderableCount: string; selectAll: string; unselectAll: string; noMatches: string; includeFile: string;
  };
  options: {
  ...
  status: {
    unableCreate: string; selectAtLeastOne: string; cancelled: string; partial: string;
...
export interface ScopeViewFeatureTranslations {
  ...
  unableOpen: string; outsideWorkspace: string; linkMenu: string; openInBrowser: string; copyLink: string; openAsScope: string; openFile: string;
}
...
const TEXT = {
  exportCenter: {
    ...
    options: {
      ...
      fullExplorerLayout: l('Full Markdown Explorer layout', 'Bố cục Markdown Explorer đầy đủ', 'Mise en page Markdown Explorer complète', 'Diseño completo de Markdown Explorer', '完整 Markdown Explorer 布局', 'Fullt Markdown Explorer-oppsett', '完全な Markdown Explorer レイアウト', '전체 Markdown Explorer 레이아웃', 'Полный макет Markdown Explorer'),
  ...
  scopeView: {
    ...
    openFile: l('Open file', 'Mở tệp', 'Ouvrir le fichier', 'Abrir archivo', '打开文件', 'Åpne fil', 'ファイルを開く', '파일 열기', 'Открыть файл'),
  },
};
