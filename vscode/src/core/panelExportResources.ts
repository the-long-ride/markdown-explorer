...
export interface PanelExportResourceInfo {
  relativePath: string;
  size: number;
}

export type PanelExportResourceListResult =
  | { ok: true; resources: PanelExportResourceInfo[] }
...
export function listPanelExportResources(workspaceRoot: string | undefined): PanelExportResourceListResult {
  const root = workspaceRoot ? workspaceRealPath(workspaceRoot) : null;
  if (!root) return { ok: false, error: 'Workspace is not available' };
  try {
    const resources: PanelExportResourceInfo[] = [];
    const visit = (directory: string) => {
      for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (isSameOrInsidePath(root, canonical)) visit(canonical);
          continue;
        }
        if (!entry.isFile()) continue;
        const canonical = fs.realpathSync(target);
        if (!isSameOrInsidePath(root, canonical)) continue;
        resources.push({ relativePath: portableRelative(root, canonical), size: fs.statSync(canonical).size });
      }
    };
    visit(root);
    ...
    return { ok: false, error: error instanceof Error ? error.message : 'Unable to list workspace resources' };
  }
}
...
export async function handlePanelExportResourceMessage(
  ...
  if (message.command === 'listWorkspaceExportResources') {
    const result = listPanelExportResources(workspaceRoot);
    await respond({ command: 'workspaceExportResourcesResult', requestId: message.requestId ?? '', ...result });
    return true;
  }
