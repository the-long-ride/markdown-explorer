...
export function createPanelGitHistoryAdapter({
  ...
  async function listRepositoryHistory({ workspacePath, limit, offset }: { workspacePath: string; limit?: number; offset?: number }) {
    const { repositoryRoot } = await resolveContext(workspacePath);
    const headOid = (await runGit(repositoryRoot, ['rev-parse', 'HEAD'])).trim();
    const output = await runGit(repositoryRoot, ['log', '--all', '--format=%x1e%H%x1f%P%x1f%an%x1f%aI%x1f%s%x1f%D', `--skip=${normalizeOffset(offset)}`, '-n', String(normalizeLimit(limit))]);
...
export async function handlePanelGitHistoryMessage(
  ...
  try {
    if (msg.command === 'getGitCapability') {
      ...
      const commits = await defaultAdapter.listRepositoryHistory({ workspacePath: root, limit: msg.limit, offset: msg.offset });
